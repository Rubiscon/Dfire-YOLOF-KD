"""CPU smoke test for dLdx + spatial AT + straight-through InfoMax matching."""
...
assert module.match == "straight_through"
assert module.differentiable_assignment
assert module.pool.output_size == 5  # x6 @ 20, proposal area reduction /16 => side /4
assert all(p.requires_grad for p in module.key_enc.parameters())
assert all(p.requires_grad for p in module.query_enc.parameters())
...
assert model.last_dict_match_stats is not None
total.backward()
...
print("InfoMax integration smoke passed:", model.last_dict_match_stats)
