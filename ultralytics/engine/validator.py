...
class BaseValidator:
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.training:
            ...
            # Optional ``model`` override: same in-training val path (EMA, no AutoBackend fuse) on a submodule.
            val_model_override = model is not None
            if not val_model_override:
            ...
            self.loss = (
                None
                if val_model_override
                else torch.zeros_like(trainer.loss_items, device=trainer.device)
            )
