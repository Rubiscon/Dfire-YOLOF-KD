...
class DetectionModel(BaseModel):
    ...
    def __deepcopy__(self, memo):
        """Clear YOLOF/DCN ``last_feature`` caches before deepcopy (ModelEMA init, checkpoint save).

        AMP checks and forward passes cache non-leaf tensors on distill blocks; PyTorch refuses to
        deepcopy them. Solo DetectionTrainer hits this path; YOLOFDistillationModel has its own copy.
        """
        saved = [
            (m, m.last_feature)
