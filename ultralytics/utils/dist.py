...
def _repo_root_for_trainer(trainer: BaseTrainer) -> str | None:
    """Parent directory of the ``ultralytics`` package that defines the trainer class."""
    import inspect
    from pathlib import Path

    try:
        path = Path(inspect.getfile(trainer.__class__)).resolve().parent
...
def generate_ddp_file(trainer: BaseTrainer) -> str:
    ...
    repo_root = _repo_root_for_trainer(trainer)
    path_preamble = ""
    if repo_root:
        path_preamble = f"""
import sys
_sys_path_root = {repo_root!r}
...
{path_preamble}from pathlib import Path, PosixPath  # For model arguments stored as Path instead of str
overrides = {overrides}
