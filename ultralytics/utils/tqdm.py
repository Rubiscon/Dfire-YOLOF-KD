...
def is_noninteractive_console() -> bool:
    ...
    In non-interactive consoles the carriage-return in-place refresh does not work, so each progress
    update is emitted on a new line and floods the log. We treat the following as non-interactive and
    print only one summary line per bar instead:
      - GitHub Actions / RunPod CI environments;
      - an explicit ``YOLO_TQDM_NONINTERACTIVE=1`` override;
      - any run whose stdout is not a real TTY (Jupyter notebooks, ``nohup``, redirected/piped logs).
    Set ``YOLO_TQDM_NONINTERACTIVE=0`` to force the live in-place bar back on.
    """
    override = os.getenv("YOLO_TQDM_NONINTERACTIVE")
